...
function calculatePricing() {
  ...
  const starterPrice = Math.round(
    baseStarterPrice *
      Math.min(teamMultiplier, 2) *
      Math.min(storageMultiplier, 1.5)
  );
  const proPrice = Math.round(
    baseProPrice * Math.min(teamMultiplier, 3) * Math.min(storageMultiplier, 2)
  );
  const enterprisePrice = Math.round(
    baseEnterprisePrice * teamMultiplier * storageMultiplier
  );
